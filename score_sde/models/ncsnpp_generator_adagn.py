...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_emd = nn.Sequential(
            nn.Embedding(self.class_num+1, self.class_num),
            nn.Linear(self.class_num, self.class_num),
            nn.SiLU(),
            nn.Linear(self.class_num, self.class_num)
        )
        if config.image_size==128 or config.image_size==256 or config.image_size==512:
            self.c_proj = nn.Sequential(
                nn.SiLU(),
                nn.Linear(self.class_num, 128),
                nn.SiLU(),
                nn.Linear(128, 256)
            )
    ...
    def forward(self, x, time_cond, z , c = None,train=True,uncondition=False, change_uncond = None):
        c_embd = self.c_emd(c)
        if train:
            c_embd[change_uncond] = 0
        if uncondition:
            c_embd = torch.zeros(c_embd.shape,device=x.device)
        cemb = self.c_proj(c_embd)
        # timestep/noise_level embedding; only for continuous training
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                h = modules[m_idx](hs[-1], temb, zemb,cemb)
                m_idx += 1
        ...
        h = hs[-1]
        h = modules[m_idx](h, temb, zemb,cemb)
        m_idx += 1
        h = modules[m_idx](h)
        m_idx += 1
        h = modules[m_idx](h, temb, zemb,cemb)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                h = modules[m_idx](torch.cat([h, hs.pop()], dim=1), temb, zemb,cemb)
                m_idx += 1
