...
class DownConvBlock(nn.Module):
    def __init__(
        ...
        if isc_emd:
            self.dense_t2 = dense(t_emb_dim,out_channel)
    ...
    def forward(self, input, t_emb,c_emb):
        out = self.act(input)
        out = self.conv1(out)
        out += self.dense_t1(t_emb)[..., None, None]
        if self.isc_emd:
            out += self.dense_t2(c_emb)[..., None, None]
        out = self.act(out)
...
class Discriminator_small(nn.Module):
  ...
  def __init__(self, nc = 3, ngf = 64, t_emb_dim = 128, act=nn.LeakyReLU(0.2)):
    ...
    self.conv5 = DownConvBlock(ngf*8, ngf*16, t_emb_dim = t_emb_dim, downsample=False,act=act)
    
    self.final_conv = conv2d(ngf*16, ngf*8, 3,padding=1, init_scale=0.)
  ...
  def forward(self, x, t, x_t, c, change_uncond=None):
    ...
    h2 = self.conv2(h1,t_embed,c_embed)
    h3 = self.conv3(h2,t_embed,c_embed)
    
    out = self.conv4(h3,t_embed,c_embed)
    out = self.conv5(out,t_embed,c_embed)
...
class Discriminator_large(nn.Module):
  ...
  def __init__(self, nc = 1, ngf = 32, t_emb_dim = 128, act=nn.LeakyReLU(0.2),
    ...
    self.c_emd = nn.Sequential(
        nn.Embedding(config.class_num+1, config.class_num),
    ...
    self.c_proj = nn.Sequential(
        nn.SiLU(),
        nn.Linear(config.class_num, 64),
        nn.SiLU(),
        nn.Linear(64, t_emb_dim)
    )
      
    self.start_conv = conv2d(nc,ngf*2,1, padding=0)
    self.conv1 = DownConvBlock(ngf*2, ngf*4, t_emb_dim = t_emb_dim, downsample = True, act=act,isc_emd = True)
    
    self.conv2 = DownConvBlock(ngf*4, ngf*8,  t_emb_dim = t_emb_dim, downsample=True,act=act,isc_emd = True)

    self.conv3 = DownConvBlock(ngf*8, ngf*8,  t_emb_dim = t_emb_dim, downsample=True,act=act,isc_emd = True)
    
    
    self.conv4 = DownConvBlock(ngf*8, ngf*8, t_emb_dim = t_emb_dim, downsample=True,act=act,isc_emd = True)
    self.conv5 = DownConvBlock(ngf*8, ngf*8, t_emb_dim = t_emb_dim, downsample=True,act=act,isc_emd = True)
    self.conv6 = DownConvBlock(ngf*8, ngf*16, t_emb_dim = t_emb_dim, downsample=True,act=act,isc_emd = True)
    self.conv7 = DownConvBlock(ngf*16, ngf*16, t_emb_dim = t_emb_dim, downsample=False,act=act)

  
    self.final_conv = conv2d(ngf*16+1, ngf*8, 3,padding=1)
    self.end_linear = dense(ngf*8, 1)
  ...
  def forward(self, x, t, x_t, c, change_uncond=None):
    ...
    h = self.conv1(h,t_embed,c_embed)
    h = self.conv2(h,t_embed,c_embed)
    h = self.conv3(h,t_embed,c_embed)
    h = self.conv4(h,t_embed,c_embed)
    h = self.conv5(h,t_embed,c_embed)

    out = self.conv6(h,t_embed,c_embed)
    out = self.conv7(out,t_embed,c_embed)
